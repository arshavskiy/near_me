  ...
  app.geoDataFull.forEach((card) => {
    ...
    if (app.geoDataFull[card.id].img) {
      ...
      L.marker([app.geoDataFull[card.id].lat, app.geoDataFull[card.id].lon], {
        ...
        .addTo(mymap)
        .bindPopup("<b>" + app.geoDataFull[card.id].title + "</b>")
        .openPopup();
    } else {
      L.marker([app.geoDataFull[card.id].lat, app.geoDataFull[card.id].lon])
        .addTo(mymap)
...
let initLeafMap = function () {
  ...
  function success(handle) {
    ...
    if (
      app.Tlatitude != handle.coords.latitude &&
      app.Tlongitude != handle.coords.longitude
    ) {
      updateGpsData(handle.coords);
    }

    if (typeof mymap == "undefined") {
      window.mymap = L.map("mapid").setView(
        [handle.coords.latitude, handle.coords.longitude],
        15
      );
